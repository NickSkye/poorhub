...
if(!isset($_GET['install']))
{
    echo '<div class="install-widget">
        <p>Note : Only 5.0 and 5.1 version can be upgrade to '.$installing_version.'. Older version need fresh installation.</p>
    ...
    <p><a class="btn" href="upgrade_5.2.php?install=1">Yes do it</a></p>
    </div>';
}
else
{
    ...
    if(@is_writable('../includes/config.php'))
    {
        ...
        $q = "DROP TABLE IF EXISTS `".$config['db']['pre']."custom_data`";
        @mysqli_query($con,$q) or install_error('ERROR ('.mysqli_error($con).')');

        echo "Creating custom_data Table...  \t\t";
