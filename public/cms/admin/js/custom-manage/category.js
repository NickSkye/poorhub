jQuery(function($) {
    ...
    $(document)
        ...
        .on('click', '.quickad-cat-lang-edit', function(e) {
            ...
            $('#editMainCategory #displayData').html('');
            $('#editMainCategory').modal('show');
            $('#editMainCategory .loader').show();
            ...
            $.post(ajaxurl+'?action=langTranslation_FormFields', data, function(response) {
                if(response != "") {
                    $('#editMainCategory #displayData').html(response);
                    $('#editMainCategory .loader').hide();
                }else{
                    quickadAlert({error: ['Problem in saving, Please try again.']});
                }
            });
...
$('#editMainCategory').on('click', '#saveEditLanguage', function() {
    var $this = $(this),
        $item = $this.closest('#editMainCategory'),
        id = $('#editMainCategory #category_id').val(),
        type = $('#editMainCategory #category_type').val();

    var $selected = {};
        jQuery(".title_code").map(function(){
        var $title = jQuery(this).val(),
            $code = jQuery(this).data('lang-code');
            $selected[$code] = $title;
    }).get();
